...
def slack_get_config(admin: AdminUser = Depends(get_current_admin)) -> dict:
    ...
    return config_response(
        "slack", cfg, source,
        "DB values require SLACK_* env update + restart to apply" if source != "env" else "live env values in effect",
...
def slack_put_config(req: SlackUpdateRequest, admin: AdminUser = Depends(require_l5)) -> dict:
    ...
    if ok:
        ...
        return config_response(
            "slack", cfg, "db",
            "saved; update SLACK_* env on the host and restart services to apply",
            effective_config=_env_config(),
        )
    ...
    return config_response(
        "slack", cfg, "in-memory",
        "saved in-memory only (dev); update SLACK_* env and restart to apply",
        effective_config=_env_config(),
    )
