...
def ol_get_config(admin: AdminUser = Depends(get_current_admin)) -> dict:
    ...
    return config_response(
        "outline", cfg, source,
        "DB values require OUTLINE_* env update + restart to apply" if source != "env" else "live env values in effect",
...
def ol_put_config(req: OutlineUpdateRequest, admin: AdminUser = Depends(require_l5)) -> dict:
    ...
    if ok:
        ...
        return config_response(
            "outline", cfg, "db",
            "saved; update OUTLINE_* env on the host and restart services to apply",
            effective_config=_env_config(),
        )
    ...
    _inmem = dict(cfg)
    return config_response(
        "outline", cfg, "in-memory",
        "saved in-memory only (dev); update OUTLINE_* env and restart to apply",
        effective_config=_env_config(),
    )
