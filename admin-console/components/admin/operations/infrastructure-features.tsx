...
import { ConfirmDialog, EmptyState, ErrorState, Skeleton, StatusBadge } from "@/components/admin";
...
export function serviceDisplayLabel(
  row: Pick<InfrastructureRow, "display_name" | "service" | "name">,
): string {
  return row.display_name || row.service || row.name || "";
}
...
export function ServicesRegistryFeature() {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? <Skeleton variant="table" rows={5} ariaLabel={t("admin.loading.content")} /> : rows.length === 0 ? <EmptyState title={t("infra.noData")} description={t("admin.operations.services.description")} /> : <Card><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>{t("infra.service")}</TableHead><TableHead>{t("admin.operations.services.target")}</TableHead><TableHead>{t("infra.healthPath")}</TableHead><TableHead className="text-right">{t("common.actions")}</TableHead></TableRow></TableHeader><TableBody>{rows.map((row) => <TableRow key={row.id}><TableCell className="font-medium">{serviceDisplayLabel(row)}</TableCell><TableCell className="font-mono text-xs">{row.host}:{row.port}</TableCell><TableCell className="font-mono text-xs">{row.health_path || "-"}</TableCell><TableCell><div className="flex justify-end gap-1"><Button type="button" size="icon" variant="ghost" aria-label={t("common.edit")} onClick={() => startEdit(row)}><Pencil aria-hidden="true" className="h-4 w-4" /></Button><Button type="button" size="icon" variant="ghost" aria-label={t("common.delete")} onClick={() => setDeleting(row)}><Trash2 aria-hidden="true" className="h-4 w-4" /></Button></div></TableCell></TableRow>)}</TableBody></Table></CardContent></Card>}
      <ConfirmDialog open={Boolean(deleting)} title={t("admin.operations.services.deleteTitle")} description={t("admin.operations.services.deleteDescription")} targetLabel={deleting ? serviceDisplayLabel(deleting) : undefined} confirmLabel={t("common.delete")} tone="danger" onConfirm={remove} onOpenChange={(open) => { if (!open) setDeleting(undefined); }} />
    </div>
  );
}
