...
- **목표**: `env_gate`를 단일 패키지 `packages/env-gate` 또는 `packages/agent-runtime` 정본으로 승격하고 타 서비스는 `import`로만 참조. drift는 CI `grep -r "OAOS_ENV.*production" --exclude=env_gate.py` 로 차단.
...
|---|---|---|---|---|
| C1 | **Critical** | Security API 무인증 노출 | `security/app.py` 5개 엔드포인트가 인증 없이 호출 가능 | 모든 Security API는 mTLS 또는 `Authorization: Bearer <CP-issued JWT>` 필수 |
| H1 | High | CP Identity Spoofing (`X-User-Id` 신뢰) | `control-plane/app.py:184` `X-User-Id` 헤더만으로 소유자 결정 | CP는 `Authorization: Bearer` 검증 + `X-User-Id == token.sub` 강제, 서명된 AgentContext 발급 |
...
| H5 | High | Distributed quota/state per-replica | `llm_runtime.py:433-569`, `admin-console/backend/llm_providers.py:946-1050` in-memory dict, `session.py:InMemorySessionStore` 기본 | Redis Lua/SET NX primary, `local+Redis` 이중화 금지·또는 원자성 보장, prod에서 Redis 미가용 시 503 |
...
| H7 | High | Mock/Fallback prod 차단 불완전 | `llm_runtime.py:901-912 _is_mock_allowed`, `mcp_client.py:60`, `proxy.py _Noop limiter` 가 env에 따라 여전히 mock 반환 가능, `OAOS_MOCK_FALLBACK=1` 로 prod 우회 가능 | prod에서 `mock/fallback/noop` 경로는 컴파일/기동 시 제거 또는 `503`, env override는 `OAOS_ALLOW_MOCK_IN_PROD` 명시 키로 한정 + 감사 로그 |
...
1. **I-C1-1**: `OAOS_ENV=production` 에서 Security의 모든 `/v1/*` 는 `Authorization: Bearer <JWT>` 또는 `mTLS client cert(CN=control-plane|execution-gateway)` 중 하나를 만족하지 않으면 `401` — **인증 mandatory, prod에서는 어떤 env로도 우회 불가**.
2. **I-C1-2**: `non-prod` 에서도 기본은 401. `OAOS_SECURITY_ALLOW_ANON=1` 은 **non-prod/test 전용**으로만 허용되며 `WARNING [fail-open] component=security reason=anon_allowed` 를 남긴다. **prod Helm/manifest에는 이 env가 존재하지 않으며 코드도 prod에서 무시**.
...
1. **Phase 1 (non-breaking, non-prod only)**: Security에 `verify_security_auth` dependency 추가하되 `OAOS_SECURITY_ALLOW_ANON=1` 은 **non-prod Helm values에만** 기본으로 배포. CP/EGW에 JWT 발급 로직 추가 후 Security 호출부에 헤더 부착. 로그로 `anon_allowed` 카운트.
2. **Phase 2 (enforce)**: `OAOS_ENV=production` 에서는 `OAOS_SECURITY_ALLOW_ANON` 값과 무관하게 항상 401 강제 (prod 코드가 env를 읽지 않음). Helm `values.prod.yaml` 에서 `OAOS_SECURITY_ALLOW_ANON` 제거 및 prod 이미지에서 분기 제거.
3. **DB/시크릿**: `OAOS_SIGNING_KEY` 는 이미 `security/app.py:36-40` 에서 prod fail-closed. 추가 시크릿 없음.

### 2.6 롤백

- **prod 롤백(안전)**: `helm rollback security <prev-revision>` 또는 이전 **서명된** Security 이미지로 `kubectl rollout undo deploy/security` — anon 허용 env로 롤백하지 않음. 트래픽은 ingress에서 `maintenance mode` 또는 `rate-limit 0` 으로 차단 가능.
- CP JWT 발급 장애 시 `mTLS` 경로로만 fallback — `security/app.py` 에서 `if mTLS_verified: pass` 분기. **평문/anon env 생성 금지**.
...
2. CP에 `verify_user_jwt` dependency 추가, 기존 `X-User-Id` 단독 경로는 **non-prod에서만** `OAOS_CP_ALLOW_PLAINTEXT_IDENTITY=1` 일 때 `WARNING` 후 허용. **prod 빌드/Helm에는 이 env가 없으며 코드가 prod에서 해당 분기를 무시**.
3. 프론트/ mattermost adapter는 `Authorization` 헤더를 붙이도록 수정 (`control-plane/control_plane/mattermost_adapter/webhook.py`).

### 3.6 롤백

- **prod 롤백(안전)**: 이전 **서명된** CP 이미지로 `helm rollback control-plane <prev>` 또는 `kubectl rollout undo deploy/control-plane` — plaintext env로 롤백 금지. 필요 시 ingress 트래픽 차단(`maintenance mode`)으로 fail-closed 유지.
...
2. **I-H2-2**: `non-prod` 에서 평문 허용 시 `WARNING [fail-open] component=egw reason=plaintext_context` + `X-Agent-Context-JWT` 우선.
...
2. EGW에 `verify_agent_context_jwt` 추가, 평문 경로는 **non-prod에서만** `OAOS_EGW_ALLOW_PLAINTEXT_CONTEXT=1` (non-prod 기본) 일 때만 허용. prod 코드는 이 env를 무시.
3. `deploy/k8s` ConfigMap: prod에는 `OAOS_EGW_ALLOW_PLAINTEXT_CONTEXT` 키 자체를 두지 않음(이미지 레벨에서 평문 경로 제거).

### 4.6 롤백

- **prod 롤백(안전)**: 이전 **서명된** EGW 이미지로 `helm rollback execution-gateway <prev>` 또는 `kubectl rollout undo deploy/execution-gateway` — plaintext env로 롤백 금지. CP JWT 발급 장애 시 prod는 `503` fail-closed(평문 fallback 없음). non-prod에서만 평문 fallback 허용.

### 4.7 테스트 acceptance

- `tests/test_egw_signed_context.py` 7건: `test_plaintext_rejected_in_prod`, `test_valid_jwt_accepted`, `test_tenant_mismatch_403`, `test_expired_401`, `test_tampered_payload_401`, `test_capability_session_binding`, `test_nonprod_plaintext_warn`.
- `execution-gateway` E2E: `curl -H "X-Agent-Context: {...}"` → prod 401, JWT로 200.
...
2. `memory_service` 에 JWT 검증 middleware 추가, 기존 body `tenant_id` 경로는 `non-prod`에서만 `WARNING` 후 허용.
3. Vault FS 함수는 `wiki_jwt` 인자를 optional로 시작, `OAOS_WIKI_REQUIRE_JWT` 는 **non-prod 과도기 전용** (`0`→`1`). **prod에서는 항상 JWT 검증 mandatory이며 env 값과 무관하게 401/403** — Helm prod에는 해당 env 미포함.

### 5.6 롤백

- **prod 롤백(안전)**: 이전 **서명된** `memory_service`/`personal-wiki` 이미지로 `helm rollback` 또는 `kubectl rollout undo` — `OAOS_WIKI_REQUIRE_JWT=0` env로 prod 롤백 금지(보안 약화). 필요 시 트래픽 차단(maintenance mode)으로 fail-closed.
...
1. **I-H4-1**: `OAOS_ENV=production` 에서 `checks.db==degraded OR checks.redis==degraded` 이면 `/readyz` 는 `503 {status:degraded, ...}` — **prod에서는 어떤 env로도 200으로 완화 불가, 503 mandatory**. `non-prod` 에서는 `200 degraded` + `WARNING` 유지 (하위호환).
...
- **prod 롤백(안전)**: 이전 **서명된** 이미지로 `helm rollback <release> <prev>` 또는 `kubectl rollout undo deploy/<svc>` — `OAOS_ENV=staging` 변경이나 `OAOS_READINESS_STRICT=0` env 우회로 prod 롤백 금지. 트래픽은 `readiness 503` 으로 자동 차단되며, 긴급 시 ingress `maintenance mode` 로 명시적 차단.
...
- `control-plane/control_plane/session.py:88-140 InMemorySessionStore` — 기본 `session_store = InMemorySessionStore()` (`session.py:285`). `RedisSessionStore` 는 `OAOS_SESSION_BACKEND=redis` 일 때만, 그것도 `fallback=True` 로 in-memory fallback.
...
1. **I-H5-1**: `prod` 에서 quota는 **Redis Lua 원자 증가+윈도 체크**가 primary — **구현·kind 2-replica 검증 완료 전까지 문서/코드에서 원자성 보장 주장 금지, 현재는 per-replica in-memory(비원자)임을 명시**. `local dict`는 `non-prod` 전용. Redis 미가용 시 `503 QUOTA_BACKEND_UNAVAILABLE` fail-closed (현재 `llm_runtime.py:523-543` 의 prod 503 분기는 유지·확대).
2. **I-H5-2**: Session은 `prod`에서 `RedisSessionStore` mandatory — `OAOS_SESSION_BACKEND=redis` 기본, `fallback=False`, **session state는 Redis에 영속, in-memory는 non-prod만**. `InMemory` 는 `non-prod`만.
3. **I-H5-3**: Token replay는 `Redis SET NX ex=ttl` 로 분산 원자성. `in-memory set`은 `non-prod` 보조만. **원자성 주장은 Redis 구현 머지 후에만**.
...
1. `admin-console/backend/llm_providers.py` 및 `llm_runtime.py` 의 in-memory 경로를 `if is_production(): raise 503` 로 가드, Redis Lua로 교체.
2. `control-plane` Deployment에 `OAOS_SESSION_BACKEND=redis`, `REDIS_URL` 주입, `RedisSessionStore(fallback=False)` 로 전환. 기존 `InMemory` 세션은 TTL 24h 후 자연 소멸 — 마이그레이션 스크립트 불필요.
3. `security/token` 은 `REDIS_URL` 필수 검증 이미 존재 (`_require_redis_if_prod`) — 유지, `OAOS_ALLOW_TEST_FALLBACK` 은 **non-prod/test 전용, prod 코드에서 무시**.

### 7.6 롤백

- **prod 롤백(안전)**: Redis 장애 시 **503 fail-closed 유지** — `OAOS_QUOTA_BACKEND=memory` / `OAOS_SESSION_BACKEND=memory` env로 prod 롤백 금지. 복구는 **Redis 의존성 복구**(Sentinel/Cluster failover, `helm rollback redis`) 또는 이전 **서명된** 앱 이미지로 `helm rollback`. 트래픽은 readiness 503으로 자동 차단.
- **non-prod에서만** `OAOS_QUOTA_BACKEND=memory` / `OAOS_SESSION_BACKEND=memory` 로 일시 우회 가능(감사 로그 필수). prod에서는 불가.
...
3. `deploy/scripts/verify-network-policy.sh` 추가 — `kind create cluster --config kind-cni.yaml`, `kubectl apply -f networkpolicy.yaml`, `kubectl run tester --image=busybox --command -- nc -zv postgres 5432` → `DROP` 확인.

### 8.6 롤백

- **prod 롤백(안전)**: 이전 **서명된** NetworkPolicy/CiliumNetworkPolicy 리비전으로 `helm rollback <release> <prev>` — `kubectl delete -f networkpolicy.yaml` 로 전체 개방 금지. 긴급 시 `CNI` 설정 복구 또는 ingress `maintenance mode` 로 트래픽 차단. 롤백 시 `WARNING` audit.
...
- `llm_runtime.py:901 _is_mock_allowed` 가 `OAOS_MOCK_FALLBACK` env로 prod에서도 mock을 허용. `packages/agent-runtime/agent_runtime/providers/*` 의 `if OAOS_ENV==production` 가드는 provider별 분산, 누락 시 mock fallback.
...
- `packages/agent-runtime/agent_runtime/env_gate.py:28 is_mock_allowed` — `OAOS_MOCK_FALLBACK` 이 `1/true` 이면 prod에서도 `True`. 기본은 prod `False` 이나 env 한 줄로 우회 가능.
- `llm_runtime.py:1530-1532` — `if not _is_mock_allowed(): raise RuntimeError("mock fallback disabled")` — env에 따라 여전히 mock 반환.
- `mcp_client.py:37-60` — `_is_production()` 복사본, `gateway_unreachable` 시 `non-prod` fallback.
- `execution-gateway/app.py:204-230 _get_rate_limiter` — `except Exception: class _Noop: allow=True` — import 실패가 곧 rate limit 무력화.

### 9.3 목표 불변식

1. **I-H7-1**: `prod` 에서 `mock/fallback/noop` 경로는 **기동 시 제거/503 fail-closed, 어떤 env로도 우회 불가 — `OAOS_MOCK_FALLBACK`/`OAOS_ALLOW_MOCK_IN_PROD`는 prod에서 무시**. prod 이미지는 mock 코드 경로 자체를 포함하지 않거나 `is_mock_allowed()`가 prod에서 항상 `False` . `non-prod`에서만 env로 허용.
2. **I-H7-2**: `prod`에서 `llm_runtime`·`mcp_client`·`rate_limiter` 의 mock/noop 분기는 `503` 또는 `RuntimeError` 로 fail-closed — **prod env override 없음**. `non-prod`만 `WARNING` 후 fallback.
3. **I-H7-3**: `CI`에서 `OAOS_ENV=production OAOS_MOCK_FALLBACK=1` 로 기동 시 `test_mock_blocked_in_prod` 가 503을 검증.

### 9.4 API/계약

```python
# env_gate.py (정본) — prod에서는 env와 무관하게 mock 불허
def is_mock_allowed() -> bool:
    if is_production():
        return False  # prod: 어떤 env로도 우회 불가, mock 경로 자체가 503
    # non-prod에서만 env 허용
    mf = os.getenv("OAOS_MOCK_FALLBACK","").strip().lower()
    if mf in ("1","true","yes","on"): return True
    if mf in ("0","false","no","off"): return False
    return True  # non-prod 기본 허용
# 참고: OAOS_ALLOW_MOCK_IN_PROD는 prod에서 사용하지 않음 — 과거 설계안의 prod 이중키는 폐기
```
...
2. `deploy/k8s/*.yaml`·`docker-compose.prod.yml` 에서 `OAOS_MOCK_FALLBACK` 제거, prod Helm에는 mock 관련 env 일체 미포함.
3. `helm` lint에서 `OAOS_MOCK_FALLBACK=1` + `OAOS_ENV=production` 조합을 `fail` , CI에서 `grep -r OAOS_ALLOW_MOCK_IN_PROD deploy/k8s --fail` .

### 9.6 롤백

- **prod 롤백(안전)**: 이전 **서명된** 이미지로 `helm rollback <release> <prev>` 또는 `kubectl rollout undo deploy/<svc>` — `OAOS_ALLOW_MOCK_IN_PROD=1` env로 prod 롤백 금지. 필요 시 `maintenance mode` 로 트래픽 차단.

### 9.7 테스트 acceptance

- `tests/test_mock_fallback_hardening.py` 5건: `test_prod_mock_env_alone_blocked`, `test_prod_mock_with_allow_flag_warns`, `test_nonprod_mock_allowed`, `test_rate_limiter_noop_blocked_in_prod`, `test_mcp_gateway_unreachable_503_in_prod`.
...
```text
Phase 0 — 정본화 (1일)
  env_gate 단일화 + CI grep drift 차단
  docs/architecture-v1.7.1-design.md 머지, README Residual 배너 추가

Phase 1 — 인증/서명 (2~3일) — C1+H1+H2+H3
  Security auth dependency (C1) + CP JWT (H1) + EGW JWT (H2) + Wiki JWT (H3)
  non-prod에서 ALLOW_ANON/PLAINTEXT 플래그로 병행, prod는 아직 off
  tests/test_security_auth, test_cp_identity, test_egw_signed_context, test_personal_wiki_auth 녹색

Phase 2 — readiness/분산 (2일) — H4+H5
  /readyz strict 503 (H4) + Redis quota/session/token (H5)
  kind 2-replica 분산 테스트, k6 병렬 quota 검증

Phase 3 — 네트워크/mock (1~2일) — H6+H7
  NetworkPolicy Cilium + kind 검증 (H6)
  mock prod 차단 이중키 (H7)

Phase 4 — 증거/문서 (1일) — H8
  README 등급 분리, deployment-verification 확장, Makefile verify-distributed

Phase 5 — prod enforce (반나절)
  helm values.prod.yaml 에서 ALLOW_* 플래그 제거, OAOS_ENV=production strict (prod에는 ALLOW_* env 자체가 없음)
  kind prod dry-run → staging → prod 롤아웃
  각 Phase는 1시간 내 롤백 가능 (이전 서명된 이미지/Helm rollback 또는 maintenance mode — prod env 완화 금지)
```
...
|--------|-----------|------|------|
| Security 401 대량 발생 | `helm rollback security <prev>` 또는 `kubectl rollout undo deploy/security` (서명된 이전 이미지) + ingress `maintenance mode` 로 트래픽 차단 | 1~2분 | `audit_log`에 `rollback:security_helm` — anon env 생성 금지 |
| CP JWT 검증 실패 | `helm rollback control-plane <prev>` 또는 `kubectl rollout undo deploy/control-plane` (서명된 이전 이미지) | 1~2분 | `rollback:cp_helm` — plaintext env 금지 |
| EGW 401 대량 | `helm rollback execution-gateway <prev>` 또는 `kubectl rollout undo deploy/execution-gateway` (서명된 이전 이미지) | 1~2분 | `rollback:egw_helm` — plaintext env 금지 |
| readiness 503으로 트래픽 0 | `helm rollback <svc> <prev>` (서명된 이전 이미지)로 복구 — `OAOS_READINESS_STRICT=0`/`OAOS_ENV=staging` env 완화 금지. 503은 정상 fail-closed이므로 Redis/DB 의존성 복구가 우선 | 2~5분 | `rollback:readiness_helm` |
| quota 503 Redis 장애 | **Redis 복구 우선**: `helm rollback redis <prev>` / Sentinel failover / `REDIS_URL` 복구 — `OAOS_QUOTA_BACKEND=memory` env로 prod 완화 금지. 트래픽은 readiness 503으로 자동 차단 | 5분 | `rollback:quota_redis_restore` |
| NetworkPolicy DROP 과다 | `helm rollback <release> <prev>` 로 이전 서명된 NetworkPolicy 리비전 복구 — `kubectl delete -f` 전체 개방 금지. 긴급 시 Cilium 설정 복구 또는 ingress maintenance mode | 1분 | `rollback:netpol_helm` |
| mock 차단 오탐 | `helm rollback <svc> <prev>` (서명된 이전 이미지) — `OAOS_ALLOW_MOCK_IN_PROD=1` env로 prod 완화 금지, maintenance mode로 트래픽 제어 | 1~2분 | `rollback:mock_helm` |

- 모든 prod 롤백은 `helm rollback <release> <prev>` 또는 `kubectl rollout undo` (서명된 이전 이미지) 원칙 — **prod에서 `kubectl set env` 로 인증/context/network/quota/mock을 완화하는 롤백 금지**. DB 마이그레이션은 v1.7.1에서 없으므로( quota 테이블은 v1.6.4에서 이미 존재) 스키마 롤백 불필요.
...
| H7 | `tests/test_mock_fallback_hardening.py` | 5 | dual-key |
...
```
packages/env-gate/env_gate.py          # 신설 또는 agent-runtime/env_gate.py 정본화
security/app.py                          # verify_security_auth dependency, mTLS
control-plane/control_plane/app.py       # verify_user_jwt, issue_agent_context_jwt, OAOS_CP_ALLOW_PLAINTEXT_IDENTITY
...
execution-gateway/execution_gateway/app.py # verify_agent_context_jwt, OAOS_EGW_ALLOW_PLAINTEXT_CONTEXT, strict readyz
...
deploy/k8s/configmap.yaml                # OAOS_*_ALLOW_* 플래그
...
- `docs/architecture-v1.7.1.md` — 본 설계안을 반영한 정본. `v1.7.0` 5026줄 대비 `+~400줄`. 헤더에 `Residual` 배지 유지, 해소 시 제거.
...
*끝 — 본 문서는 HEAD 201b78b6c2 기준 증거로 작성되었으며, 구현 전 2차 검증을 전제로 한다.*