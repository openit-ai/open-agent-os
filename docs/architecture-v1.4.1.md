# Open Agent OS 최종 아키텍처 및 구현 설계서 v1.4.1
...
초기 구현에서는 일반 업무용 Safe Runtime을 기본 Runtime으로 사용하고, Hermes Agent는 고복잡·고자율 업무를 위한 Advanced Runtime으로 채택한다.
...
# 16F. Dual Agent Runtime Architecture
...
Hermes가 설치되지 않은 경우 Open Agent OS는 Safe Runtime만으로 정상 동작해야 한다.
...
Safe Runtime은 Open Agent OS의 기본 Runtime이다.
...
즉 Safe Runtime은 `LLM + MCP Tool Calling + Controlled Agent Loop`에 가깝다. Tool이 존재하지 않는 행동은 수행하지 못한다.
...
```text
General Office Work                  → Safe Runtime
Sales / HR / Finance Standard Work   → Safe Runtime
...
Level 2 : LLM + MCP / No Shell          = Safe Runtime
...
```text
1. 일반 Personal Agent는 Safe Runtime을 기본값으로 사용한다.
