...
| A4 | Ledger persisted & exportable (S3 prefix) | ⚠️ Partial | `security/app.py:301-332` exposes `/v1/audit/verify|checkpoint|events`; `deploy/docker-compose.prod.yml:130` sets `AWS_S3_BUCKET/REGION/PREFIX=audit-checkpoints/` env. Persistence follow-up: `AuditLedger` inserts `AuditEventORM` through synchronous SQLAlchemy transactions and reconstructs persisted hash links on restart. DB reads reflect committed events. Restart coverage: `tests/test_audit_restart_persistence.py`; external checkpoint export remains a separate concern. |
...
| Gap | Remediation |
|---|---|
| **Approval/Audit persistence — implemented; restart regression coverage added** | Configure `DATABASE_URL` or `OAOS_DATABASE_URL`; production (`OAOS_ENV=production`) fails closed without DB. Stores use synchronous SQLAlchemy transactions with `ApprovalRequestORM` / `AuditEventORM`; memory remains the no-DB development fallback. Existing migrations `001_initial_persistence` and `008_approval_nonces` already provide the schema; no additional migration is needed. See `tests/test_approval_restart_persistence.py` and `tests/test_audit_restart_persistence.py`. |
