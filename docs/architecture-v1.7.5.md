# Open Agent OS 최종 아키텍처 및 구현 설계서 v1.7.5
...
> Version: **v1.7.5** — 2026-09-16 (v1.7.4 → v1.7.5 post-v1.7.4 merged completion record; product `0.1.10`)
> Version separation: **아키텍처 문서 `v1.7.5`와 제품 `0.1.10`은 서로 독립된 버전 체계이며, 어느 한쪽도 다른 쪽의 릴리스 번호를 의미하지 않는다.**
...
> Status: **H4/H5/H6/H7/H8 implemented (verified by git/tests) · RAG architecture and implementation complete, Personal Wiki module implemented and tested, Adaptive Profile v1.7.2 MVP implemented (Profile API/persistence/policy synthesis/Runtime Hook) · Secrets lifecycle implemented via systemd installer · Evidence tiers verified via `scripts/verify-evidence-tiers.py` (unit/distributed/external counted per run; historical v1.7.1 snapshot was unit: 927 passed — rerun on v0.1.3 candidate instead of reusing that number)** · IA aliases implemented (v1.7.3 §16.14: admin `/control/*`·`/execution/*` views + `/v1/control/acp`·`/v1/execution/mcp` aliases, canonicals kept; `oaos-adapter-mattermost`/`oaos-governance` coexistence) · Admin Console IA 전환 완료 (v1.7.4 §16.15: 6그룹 canonical routes, legacy 307 aliases, 공통 UX 킷) · v1.7.4 이후 병합 완료 내역 기록 (v1.7.5 §16.16: #19–#24, #26–#27, #29–#31; 미검증 항목은 residual 유지)**
...
### 16.14 Control-Plane-centric IA aliases + canonical process names — implemented (v1.7.3)

실측 구조(`Adapter → ACP → Control Plane → RuntimeRouter → Execution Gateway → Knowledge`)가 관리자 콘솔·프로세스명에 그대로 드러나지 않던 문제를 별칭 병행으로 해소한다. 정본 URL·API·유닛은 유지하고, 새 체계는 별칭으로 1릴리스 병행한다.

```text
Mattermost/Slack Adapter (oaos-adapter-mattermost)
  → Control Plane :8100 (identity/session/ACP adapter/policy/audit + RuntimeRouter)
  → Agent Runtime (Hermes :8642 vs external LLM)
  → Execution Gateway :8001 (MCP registry/connectors/capability/risk)
  → Knowledge (Outline/Index/Memory/Embedding)
```

- **Admin IA 6그룹**: 연결 Ingress / 제어 Control Plane / 실행 Execution / 지식 Knowledge / 운영 Operations / 관리 Admin. ACP는 Providers 하위에서 제어 그룹 1급(`/control/acp`)으로, MCP는 Infra 탭에서 실행 그룹 1급(`/execution/mcp`)으로 승격. `/control/runtime`은 `/runtime-config` 별칭. 기존 URL 무변경.
- **Backend 별칭**: `/v1/control/acp/*`(3 routes)·`/v1/execution/mcp/*`(5 routes)를 동일 endpoint 공유(`include_in_schema=False`)로 추가. 정본 `/v1/acp`·`/v1/mcp` 유지. DB 키(`admin_settings.acp_config`/`mcp_servers`) 무변경, 시크릿 write-only 유지.
- **프로세스 정식명**: `oaos-adapter-mattermost.service`(구 `oaos-mm-bridge` 병행), `oaos-governance.service`(구 `oaos-security` 병행, policy/delegation/token/approval/audit). CP·EG Description을 중심 역할 표현으로 수정. 신규 유닛은 enable/start 없이 파일 배치만(구 유닛이 계속 실행).
- **스냅샷 호환**: `_build_snapshot()`에 참조 전용 `process_aliases` 추가(시크릿 없음). 구 리더는 미지 키 무시.
- **검증**: `npm run build`(신라우트 3종 출력) · 표적 `28 passed` · `py_compile`+`bash -n` · import 레벨 8 별칭 확인 · KVM4 선별 sync 후 live HTTP 정본/별칭 `401`(auth-gated live) · console/api/CP `200` read-back.

**Completion update (v1.7.4)**: 구 라벨 정리와 6그룹 사이드바 기본 전환은 §16.15에서 완료됐다.

**Residual**: 운영 롤아웃(enable/start 전환)은 여전히 별도 승인 후 수행한다.

### 16.15 Admin Console IA 전환 완료 — canonical routes · 6그룹 nav · 공통 UX 킷 (v1.7.4)

Phase 1~6 병합으로 Admin Console의 기본 정보구조와 목록 UX를 다음 상태로 전환했다. 이 절은 병합된 코드와 테스트로 확인되는 범위만 기록하며, legacy URL 삭제나 운영 서비스 전환을 완료로 주장하지 않는다.

#### IA·라우트 계약

- **6그룹 + 고정 2항목**: 고정 항목은 대시보드 `/`, 시작하기 `/setup`이다. 그룹은 연결 `/connections/*`, 설정 `/control/*`, 실행 `/execution/*`, 지식 `/knowledge/*`, 모니터링 `/operations/*`, 관리 `/management/*`로 구성한다. 데스크톱 사이드바와 모바일 drawer는 같은 `AdminNavigation` 컴포넌트를 공유해 동일한 계층을 표시한다. 내비게이션 링크는 30개이며 legacy 경로는 0개다.
- **canonical 승격**: Phase 3~5에서 `/management/users|credentials|secrets|feature-flags|profile-operations`, `/operations/backup|security-updates|license`, `/knowledge/operations|embedding`, `/control/policy|approvals|audit`, `/execution/providers|fallback|usage|quota`를 canonical로 승격했다.
- **307 alias 정책**: 단순 legacy URL 17개는 `next.config.js`의 `redirects()`와 `permanent: false`를 사용해 서버에서 307로 이동하며 query string을 자동 보존한다. `/infra`는 tab/hash를 해석하고, `/providers`는 `#acp`를 `/control/acp`로 분기해야 하므로 클라이언트 resolver를 유지한다. 서버는 fragment를 받지 못하므로 이 두 URL에 일괄 서버 redirect를 적용하면 fragment 확인 전에 이동해 호환성이 깨진다.
- **정리 제외**: `/setup`, `/control/acp`, `/execution/mcp`는 canonical URL로 계속 유지한다.
- **라우트 삭제 없음**: legacy client page 삭제는 0건이며 롤백 수단으로 보존한다.

#### 공통 UX·상태·접근성 계약

- **공통 UX 킷(Phase 1)**: `DataTable`, `Dialog`, `ConfirmDialog`, `Toast`/`useToast`, `FormField`, `Skeleton`, `EmptyState`, `ErrorState`, `StatusBadge`, `useTableQuery`를 도입했다. 목록은 검색·정렬·페이지·URL query 보존 흐름을 공유한다. 파괴적 동작은 `ConfirmDialog`를 사용하며 고위험 동작은 텍스트 재입력을 요구한다. native `alert`/`confirm`/`prompt` 호출은 0건이다.
- **상태 배타성**: 목록 화면의 `loading / error / empty / data / filtered-empty / background-error` 상태는 서로 배타적이다.
- **i18n**: 새 키는 `admin.<feature>.<component>.<meaning>` 구조로 ko/en에 동시에 추가한다. 기존 `nav.*` 키의 의미와 이름은 바꾸지 않으며, ko/en key set 일치 테스트가 이를 검사한다.
- **접근성**: 내비게이션 그룹은 `role="group"`과 `aria-labelledby`, 활성 링크는 `aria-current="page"`, 키보드 포커스는 `focus-visible`, 아이콘은 `aria-hidden`을 사용한다. drawer 버튼은 `aria-expanded`와 `aria-controls`를 제공한다.
- **빌드 불변식**: `useSearchParams()`를 사용하는 화면은 반드시 `<Suspense>` 경계 안에 둔다. 경계가 없으면 Next.js 15 프리렌더가 중단되어 빌드가 실패하며, Phase 4에서 실제 배포 차단 사례가 발생했다.

#### 검증·URL 정리·잔여

- **검증 게이트**: Admin Console 변경은 `npx tsc --noEmit`(exit 0), `npx vitest run`, `npm run build`(exit 0), `npx eslint .`(exit 0)와 Python `py_compile`을 통과해야 한다. URL smoke는 canonical `200`, legacy `307`과 query 보존을 확인하고, 내비게이션 링크 전수를 crawl한다.
- **URL 정리 정책**: 최소 1회 릴리스 주기와 마지막 접근 후 30일이 모두 지난 뒤 `/var/log/nginx` 실측 접근이 0건인 URL만 정리 후보가 된다. 정리에도 마스터 승인이 필요하다. 절차는 [`docs/admin-ux-legacy-url-runbook.md`](admin-ux-legacy-url-runbook.md)를 따른다.
- **Residual (v1.7.4)**: ① 목록 API의 additive 서버 검색·정렬·페이지 계약(현재 클라이언트 호환 어댑터) ② 기능 플래그 영구 감사 이력 ③ 관찰 기간 후 legacy URL 정리 ④ 운영 롤아웃.

참조: [`docs/admin-ux-redesign-v1.0.md`](admin-ux-redesign-v1.0.md) (설계 정본), [`docs/admin-ux-legacy-url-runbook.md`](admin-ux-legacy-url-runbook.md) (운영 runbook), [`docs/admin-ia-control-plane-redefinition.md`](admin-ia-control-plane-redefinition.md).

### 16.16 v1.7.4 이후 병합 완료 기록 — 런타임 호환성 · setup/readiness · 제어 화면 · 배포 · 감사 · 테스트 격리 (v1.7.5)

이 절은 v1.7.4 문서 확정 이후 `main`에 병합된 구현 중 코드·테스트·보고서로 확인된 범위만 완료로 기록한다. 제품 버전 `0.1.10`과 아키텍처 문서 버전 `v1.7.5`는 독립적이다.

| PR / merge | 완료로 확인된 범위 |
|---|---|
| #19 `bc648fd` | 운영 Node 22.22.1보다 높은 patch를 요구하던 jsdom 30 대신 Node 22.13 호환 jsdom 29 계열을 고정하고 Admin Console Node 하한을 명시했다. |
| #20 `873a634` | 주기 infra probe 기동·종료, readiness 관찰 DB 영속화, canonical registry idempotent seed, revision 기반 applied 판정, ko/en setup blocker 안내, incomplete 상태 렌더링을 구현했다. |
| #21 `a1c9f52` | `OAOS_INFRA_SEED_EXCLUDE`를 startup·수동 seed·통합 목록의 공통 opt-out으로 적용하고 Outline probe 경로를 `/_health`로 교정했다. |
| #22 `c10574c` | policy·approvals·audit의 화면 문자열과 상태·동작 라벨을 ko/en 카탈로그로 일원화하고 locale별 렌더링 검증을 추가했다. |
| #23 `1979881` | 세 제어 화면 본문을 `components/admin/control/control-features.tsx`로 추출하고 route를 `Suspense` wrapper로 정렬했으며 파생 배열을 memoize해 lint 경고를 해소했다. |
| #24 `e512244` | 미참조 카탈로그 275키를 제거해 ko/en 각각 1,060키로 맞추고 literal·dynamic i18n lookup의 누락 키를 실패시키는 테스트를 추가했다. |
| #26 `bb16481` | systemd admin-api의 `PYTHONPATH`에 실제 import하는 `audit_model`, `delegation_model`, `mcp_resource_model`, `agent_runtime` 패키지 경로를 추가했다. |
| #27 `351ee4b` | 감사 append가 저장된 tip에서 이어지도록 하고 프로세스·PostgreSQL 수준에서 직렬화했으며 policy publish/approve/rollback과 감사 append를 한 트랜잭션으로 묶었다. |
| #29 `569f8a5` | 정책 테스트가 `backend.policy`를 패키지 경유로 import하도록 고쳤다. 모듈 사이 환경 복원, 실제 oaos.cloud infra 도메인 커버리지, 로드된 admin infra store 격리, EOF 공백 제거를 포함한다. |
| #30 `ac20c76` | Outline/Mattermost 기본값과 관련 fixture를 `note.oaos.cloud`·`chat.oaos.cloud`로 전환했다. 보고서의 저장소 검색에서 두 구도메인은 각각 0건이었다. |
| #31 `5d7b89e` | live registry fixture가 이미 로드된 admin app이 보관한 infra 모듈까지 초기화해 `sys.modules` 교체 뒤 남는 고아 module alias와 이전 registry row를 격리한다. |

#### 검증된 범위

- PR #19–#24의 Admin Console 결과는 각 병합 테스트와 v0.1.8 릴리스 측정에 근거한다. 최종 측정은 TypeScript exit 0, Vitest 31 files / 116 tests, ESLint exit 0, build 56/56, ko/en 카탈로그 1,060 = 1,060이었다.
- PR #27의 신규 9개 regression test는 이전 구현에서 실패하고 수정 후 통과했으며, 관련 9개 suite는 147 passed였다.
- PR #29 최종 전체-suite 순서 실행은 1551 passed, 32 skipped였고 영향받던 Outline 3건은 통과했다. 표적 4 passed, admin/infra 42 passed, 고아 alias 축약 순서 8 passed였다.
- PR #30 변경 검증은 Outline 3 passed, infrastructure UI 4 passed, TypeScript와 표적 ESLint exit 0이었다. `note.openit.co.kr`와 `chat.openit.co.kr` 저장소 잔존은 각각 0건이었다.
- PR #31 live registry 표적 suite는 5 passed였고 표적 Ruff는 통과했다.

#### Residual — 완료로 올리지 않는 항목

- PR #29 전체-suite의 8개 환경 의존 실패(personal-wiki filesystem 1, migration timeout 2, systemd installer write 5)는 해결 완료로 주장하지 않는다.
- §16.12의 post-interaction worker·Hermes critical-path 자동 주입·Profile Skill/UI/cache invalidation·distributed/external/live RAG 운영 E2E는 미검증 상태를 유지한다.
- §16.14의 프로세스 운영 롤아웃과 §16.15의 server-side 목록 계약·기능 플래그 영구 감사·legacy URL 관찰 후 정리·운영 롤아웃은 별도 승인과 실측 전까지 residual이다.
- KVM4 배포·재시작, live 외부 통합, production rollout은 이 문서 갱신에서 수행하거나 검증하지 않았다.
...
| **v1.7.5** | **2026-09-16** | **§16.16 v1.7.4 이후 병합 완료 기록 — jsdom 호환성(#19), setup/readiness(#20), infra opt-out(#21), control i18n·추출·카탈로그(#22–#24), admin-api 경로(#26), 감사 직렬화·원자성(#27), package import·store/registry 격리·oaos.cloud 기본값(#29–#31); 환경 의존 실패와 운영/live 검증은 residual (제품 `0.1.10`과 별개)** |
| **v1.7.4** | **2026-09-15** | **§16.15 Admin Console IA 전환 완료 — canonical routes·6그룹 nav·공통 UX 킷, 단순 legacy 17개 307 alias와 fragment 의존 client resolver 2건, legacy page 삭제 0건, URL 관찰·승인 정리 정책 (제품 `0.1.7`)** |
| **v1.7.3** | **2026-09-05** | **§16.14 Control-Plane-centric IA aliases + canonical process names — 어드민 `/control/acp`·`/control/runtime`·`/execution/mcp` 별칭뷰, 백엔드 `/v1/control/acp/*`·`/v1/execution/mcp/*` 별칭(정본 유지), 정식 유닛 `oaos-adapter-mattermost`·`oaos-governance`(구 유닛 1릴리스 병행), 스냅샷 `process_aliases` — 제품 `0.1.5`** |
...
- **§16.16 v1.7.4 이후 병합 완료 기록 — v1.7.5** (#19–#24, #26–#27, #29–#31 완료 범위와 미검증 residual 분리; 제품 `0.1.10`과 독립)
