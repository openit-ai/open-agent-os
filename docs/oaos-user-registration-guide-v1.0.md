...
OAOS 오류는 내부 원문을 사용자에게 노출하지 않고, 안전한 사용자 메시지로 정규화한다. 등록·권한·세션 확인에 실패하면 해당 요청을 즉시 종료하며 LLM·Google·Personal Wiki·Enterprise Knowledge MCP를 호출하지 않는다.

| 상황 | 사용자에게 표시되는 안내 | 재시도 |
|---|---|---|
| 사용자 매핑 없음 / 등록 필요 | 현재 OAOS 사용자 등록이 되어 있지 않습니다. 웹관리자 콘솔에서 등록 상태를 확인한 뒤 다시 시도해 주세요. | 아니오 |
| 권한 거부 | 현재 계정에는 이 작업을 수행할 권한이 없습니다. | 아니오 |
| 인증 실패 | OAOS 인증 확인에 실패했습니다. 관리자에게 문의해 주세요. | 아니오 |
| 세션·리소스 없음 | 요청한 세션 또는 리소스를 찾을 수 없습니다. 새 대화로 다시 시도해 주세요. | 아니오 |
| 요청 형식·입력값 오류 | 요청 형식 또는 입력값을 확인해 주세요. | 아니오 |
| 요청 과다·일시 장애 | 현재 요청이 많거나 연동 서비스가 일시적으로 응답하지 않습니다. 잠시 후 다시 시도해 주세요. | bounded |

영구 오류는 동일 Mattermost Post를 한 번만 처리하고 `seen`/멱등성 상태에 기록한 뒤 안내 메시지를 게시하고 종료한다. 일시 오류는 제한된 횟수와 cooldown을 적용하며 무한 재시도하지 않는다. callback state 불일치, Google profile email 불일치, scope 부족, 토큰 만료는 각각 해당 사용자 OAuth 흐름만 중단·재시작한다. 다른 사용자 token으로 대체하지 않는다. 내부 traceback·credential·파일 경로는 로그와 사용자 메시지에 남기지 않는다.
