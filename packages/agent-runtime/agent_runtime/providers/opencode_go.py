...
class OpenCodeProvider:
    ...
    async def call(self, messages: list[dict[str, Any]], model: str | None = None, tools: list[dict[str, Any]] | None = None, **kwargs: Any) -> dict[str, Any]:
        ...
        if not _is_mock_allowed():
            raise RuntimeError("LLM provider unavailable: opencode-go — mock fallback disabled in production")
                ...
                async with httpx.AsyncClient(timeout=timeout_s) as client:
                    ...
                    if http_result is not None:
                        ...
                        if tcs:
                            ...
                            return
                        if not _is_mock_allowed():
                            raise RuntimeError("LLM provider unavailable: opencode-go — mock stream disabled in production")
