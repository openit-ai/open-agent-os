...
- **Architecture:** `docs/architecture-v1.6.4.md` (Sections 1–47 + §§16A–16K + §16.1.1–16.1.2 LLM Runtime & 6-Provider Multi-Runner + §§16.4–16.6 Quota/Usage/HA + §27 Personal Wiki Vault — LLM/Hermes dual runtime, untrusted worker, tool policy, data access pattern; §§16A.3.1 workspace isolation / 16A.6 Controlled Egress Proxy new — `e10c1af8`, 4945 lines) — Control Plane / Execution Gateway / Security & Governance + Zero-Bypass Invariants / Runtime-Agnostic (Previous: [`docs/architecture-v1.5.md`](docs/architecture-v1.5.md) `b19f54ab` · [`docs/architecture-v1.4.1.md`](docs/architecture-v1.4.1.md) `646a8fe` · [`docs/architecture-v1.3.md`](docs/architecture-v1.3.md) `4a0383c8` · _v1.1 preserved as `docs/architecture-v1.1.md`_)
- **Status:** `v0.1.1` — Workstream A+B+C + MVP Demo + Admin Console (12 routes, LLM Providers 포함), `612 tests pass`, `npm run build ✓`
...
2. **진짜 격리** — `agent:assistant:kim`은 `employee:kim` 소유 자원만 본다. Cross-user는 항상 DENY, plaintext token 저장 금지, Hermes 프로세스에 장기 저장 금지(§10). 612 tests로 검증.
...
tests/                     # 612 tests (incl. LLM 6-Provider + Fernet Vault + opencode binary + wiki/pgvector)
docs/architecture-v1.6.4.md  # Canonical (47 Sections + §§16A–16K + §16.1.1–16.1.2 LLM 6-Provider + §§16.4–16.6 Quota/Usage/HA + §27B Wiki Vault — 4945 lines, SHA e10c1af8, Previous v1.6.2/v1.6/v1.5.1 preserved, v1.1 preserved)
```
...
- `docs/architecture-v1.6.4.md` — Canonical (47 Sections + §§16A–16K + §16.1.1–16.1.2 LLM 6-Provider + §§16.4–16.6 Quota/Usage/HA + §27B Wiki Vault — 4945 lines, SHA `e10c1af8`) — Previous: `docs/architecture-v1.6.2.md` `4456bd4c`/v1.6/v1.5.1 preserved
