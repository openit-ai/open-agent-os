...
**Tag/commit:** `v0.1.3` candidate `cb445fd7cb` on `release/v0.1.3-remediation` (not yet tagged; previous `v0.1.2` was `34f0981e71` on `origin/main`). Product version `0.1.3` is read from `admin-console/package.json` (env `OAOS_VERSION` overrides); architecture `v1.7.2` remains the design document version — do not conflate.

**Included in this tag (11 files, 647 insertions):**
- **Admin Web UI fixes**
  - `admin-console/lib/api.ts`: `BASE_URL` `http://localhost:8000` → `"/api"` same-origin (nginx `/api` proxy), 401 `handleUnauthorized` clears token/avatar → `/login` replace (no loop on login path), `formatApiErrorDetail` never emits `[object Object]` (array/object/message/msg/error/detail/reason/title → JSON fallback), `normalizeLLMUsage*` + `updateMapping`/`display_name`·`avatar_url` extensions.
...
- **P0 / P1 (already in main before this tag, verified present at HEAD — not re-committed)**
  - P0 `00a6fcb890` — durable Mattermost idempotency (Redis `tenant+channel+post` deterministic key, atomic claim, completed/failed_retryable, prod 503, retry bounded) + multimodal image delivery via active Agent Runtime (ACP/Hermes).
  - P1 `57e9a4fcc2` — live Knowledge Index verification (health `check_*_credentials`/`probe_*_health`, ACL pre-filter contract, `content_hash`/`acl_version` incremental sync, bounded retry/checkpoint, `scripts/verify-knowledge-live.py`).
  - P1 fix `8855441b83` — fail-closed Notion adapter handling for clean checkout (`http_notion.py` missing → deterministic blocker, no crash, 24 P1 tests pass).

**Knowledge Index / RAG boundary (honest, no over-claim):**
- **Outline — read-only verification only.** Connector `HttpOutlineSourceAdapter` is read-only, bounded single-page probes (`page_limit=1..5`), `source_reference`/`content_hash`/`acl_version` tracked. Full distributed/external RAG is **not** claimed in this release. Live corpus probe is bounded; the backing store remains the source of truth and full backfill/distributed verification requires live credentials + `scripts/verify-knowledge-live.py --health` — see P1 scope `§0.4, §16.9-16.11`. Where observed, a bounded paging probe reports up to ~869 pages with `page_limit=1` (code comment in `health.py`), indicating corpus scale without claiming bulk ingestion.
- **Notion — not live-integrated in this release.** `HttpNotionSourceAdapter` (`http_notion.py`) is read-only and fail-closed: missing module or missing credentials (`NOTION_API_KEY`/`OAOS_NOTION_TOKEN`) returns deterministic blocker `Notion adapter missing: knowledge_index/connectors/http_notion.py not present` / `Notion credentials missing ... live Notion connector not verifiable`, `verifiable=false`, `adapter_missing=true` where applicable, no mock fallback and no fabricated health. Live Notion verification requires `http_notion.py` + credentials + network — not claimed as complete.
- **Distributed/external remain 0** until live verification (`kind`+`redis`+`hubble`+`Outline/Notion/Mattermost/Slack/LLM gateway` live network) — see `scripts/verify-evidence-tiers.py` tiers. `scripts/verify-knowledge-live.py` remains the operational verification for connectors; `scripts/verify-evidence-tiers.py --check-only` enforces H8 against unsupported claims.
